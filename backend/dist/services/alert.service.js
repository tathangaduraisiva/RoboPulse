...
export async function createAlertRecord(input) {
    ...
    const dedupCheck = await pool.query(`SELECT id, severity, message, status, created_at
         FROM alerts
         WHERE robot_id = $1
           AND type = $2
           AND status NOT IN ('resolved')
         ORDER BY created_at DESC
         LIMIT 1;`, [input.robot_id, input.type]);
    if (dedupCheck.rows.length > 0) {
        ...
        if (newRank > existingRank) {
            await pool.query(`UPDATE alerts
                 SET severity = $1, message = $2, updated_at = NOW()
                 WHERE id = $3;`, [input.severity, input.message, existing.id]);
            console.log(`[AlertService] Upgraded active alert for robot ${input.robot_id} ` +
            ...
            await pool.query(`UPDATE alerts
                 ...
                 WHERE id = $2;`, [input.message, existing.id]);
        }
    ...
    if (!isOfflineType && input.severity !== 'critical') {
        const cooldownCheck = await pool.query(`SELECT id FROM alerts
             WHERE robot_id = $1
               AND type = $2
               AND status = 'resolved'
    ...
    if (!isOfflineType && (input.severity === 'critical' || input.severity === 'high')) {
        const handledCheck = await pool.query(`SELECT id FROM handled_conditions
             WHERE robot_id = $1
               AND suppression_expires_at > NOW()
             LIMIT 1;`, [input.robot_id]);
        if (handledCheck.rows.length > 0) {
            console.log(`[AlertService] Suppressing new ${input.severity} alert for robot ${input.robot_id} ` +
...
export async function autoResolveChannelAlert(robotId, type) {
    const existing = await pool.query(`SELECT id FROM alerts
         WHERE robot_id = $1
           AND type = $2
           AND status NOT IN ('resolved');`, [robotId, type]);
 ...
 */
export async function processTelemetryAlerts(robotId, robotName, vals) {
...
export async function resolveAlert(id) {
    ...
    const robotId = fetchResult.rows[0]?.robot_id;
    const query = `
        UPDATE alerts
        SET status = 'resolved', updated_at = NOW()
        WHERE id = $1
