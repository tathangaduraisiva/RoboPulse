...
export async function findOrCreateGoogleUser(googleId, email, displayName, avatarUrl) {
    ...
    const byGoogleId = await pool.query(`SELECT id, name, username, email, role
         FROM users
         WHERE google_id = $1
         LIMIT 1`, [googleId]);
    if (byGoogleId.rows[0]) {
        // Update avatar URL in case it changed (non-critical)
        await pool.query(`UPDATE users SET google_avatar_url = $1, updated_at = NOW() WHERE google_id = $2`, [avatarUrl, googleId]);
    ...
    const byEmail = await pool.query(`SELECT id, name, username, email, role
         FROM users
    ...
    if (byEmail.rows[0]) {
        ...
        await pool.query(`UPDATE users
             ...
             WHERE LOWER(email) = LOWER($3)`, [googleId, avatarUrl, safeEmail]);
    ...
    let suffix = 0;
    while (true) {
        const conflict = await pool.query(`SELECT id FROM users WHERE LOWER(username) = LOWER($1) LIMIT 1`, [username]);
        if (!conflict.rows[0])
            break;
        suffix += 1;
    ...
    const result = await pool.query(`INSERT INTO users
         ...
         RETURNING id, name, username, email, role`, [safeName, username, safeEmail, googleId, avatarUrl]);
...
export function registerGoogleStrategy() {
    ...
    passport.use(new GoogleStrategy({
        ...
        try {
            ...
            const user = await findOrCreateGoogleUser(profile.id, email, displayName, avatarUrl);
            done(null, user);
        }
        catch (err) {
            console.error("[GoogleStrategy] Error during find-or-create:", err instanceof Error ? err.message : err);
