 ...
 */
export async function invalidateRobotCache(robotId) {
    try {
 ...
 */
export async function getCurrentRobotStatusDetails(robotId) {
    // 1. Check telemetry recency (connectivity health)
    const telemetryRes = await pool.query(`SELECT recorded_at, temperature_c, vibration_mm_s, motor_current_a, pressure_bar
    ...
    await pool.query(`UPDATE alerts
         ...
         WHERE robot_id = $1
           ...
           AND status != 'resolved';`, [robotId]);
    // 2. Check for active or overdue maintenance
    const maintRes = await pool.query(`SELECT id, maintenance_type, performed_at, next_due_at, condition_handled_at, description
         FROM maintenance_records
         WHERE robot_id = $1
         ORDER BY created_at DESC;`, [robotId]);
    ...
    const alertsRes = await pool.query(`SELECT id, type, severity, status, message
         FROM alerts
         WHERE robot_id = $1
...
export async function getCurrentRobotStatus(robotId) {
 ...
 */
export async function syncRobotStatus(robotId) {
    ...
    await pool.query(`UPDATE robots
         SET status = $1
         WHERE id = $2;`, [calculatedStatus, robotId]);
 ...
 */
export async function syncAllRobotStatuses() {
    try {
