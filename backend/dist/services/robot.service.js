...
export async function getCurrentRobotStatus(robotId) {
    ...
    const alertsRes = await pool.query(`SELECT type, severity, status
         FROM alerts
         WHERE robot_id = $1
    ...
    const hasOfflineAlert = activeAlerts.some((a) => a.type?.toLowerCase() === "offline");
    if (hasOfflineAlert) {
        return "offline";
    }
    // 2. Check telemetry recency (connectivity health)
    const telemetryRes = await pool.query(`SELECT recorded_at
    ...
    const maintRes = await pool.query(`SELECT id, maintenance_type, performed_at, next_due_at, condition_handled_at
