 ...
 */
function deriveConditionCategory(vibScore, tempScore, currentScore, pressScore = 0, errScore = 0) {
...
export async function evaluateRobotTelemetry(robotId) {
    ...
    const weightedBase = 0.30 * vibScore +
        0.25 * tempScore +
        0.20 * currentScore +
        0.15 * pressScore +
        0.06 * errScore +
    ...
    const { category: conditionCategory, signalBucket } = deriveConditionCategory(vibScore, tempScore, currentScore, pressScore, errScore);
...
export async function getCurrentConditionFingerprint(robotId) {
    try {
        ...
        const presses = rows.map((r) => Number(r.pressure_bar) || 0);
