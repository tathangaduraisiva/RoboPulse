...
export async function registerUser(name: string, email: string, password: string): Promise<AuthUserSummary> {
    ...
    if (!password || !isStrongPassword(password)) {
        throw Object.assign(
            new Error("Password must be at least 8 characters and include uppercase, lowercase, and a number."),
            { statusCode: 400 }
        );
    }
    ...
    try {
        result = await pool.query(
            ...
            [safeName, normalizedEmail, normalizedEmail, passwordHash]
        );
    } catch (dbError: unknown) {
...
export async function loginUser(username: string, password: string): Promise<LoginTokenResult> {
