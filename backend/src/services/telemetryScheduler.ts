...
// ─── Per-channel nominal operating spec ──────────────────────────────────────
// Values match the spec's "normal operating region" guidance.
// Each robot gets a small per-robot offset so their baselines differ.

interface ChannelSpec {
    mean:      number;   // long-run mean
    noise:     number;   // typical tick-to-tick step (≈ 1 σ)
    revStr:    number;   // mean-reversion strength per tick [0,1]
    hardMin:   number;   // physical lower clamp
    hardMax:   number;   // physical upper clamp
    /** Moderate anomaly: target offset from mean (+ or − randomly) */
    anomalyMod: number;
    /** Rare sharp anomaly: target offset from mean */
    anomalySharp: number;
}
...
const CHANNEL_SPEC: Record<string, ChannelSpec> = {
    temp: {
...
// Per-robot baseline offsets — derived from a deterministic name hash at
// init time so each robot operates in its own range without name branches.
interface BaselineOffsets {
    temp: number;   // added to CHANNEL_SPEC.temp.mean
    vib:  number;
    cur:  number;
    pres: number;
}

// ─── Anomaly event state for one channel ─────────────────────────────────────

interface AnomalyEvent {
    /** Where we want to push the value during the event */
    target:    number;
    /** How many ticks the push phase lasts */
    pushTicks: number;
    /** How many ticks already spent in push phase */
    pushed:    number;
    /** true once push is done and we are reverting back */
    recovering: boolean;
}

// ─── Full per-robot runtime state ────────────────────────────────────────────
...
interface RobotState {
    ...
    // Active anomaly events (null = none)
    tempAnomaly: AnomalyEvent | null;
    vibAnomaly:  AnomalyEvent | null;
    curAnomaly:  AnomalyEvent | null;
    presAnomaly: AnomalyEvent | null;
...
function triangleNoise(scale: number): number {
...
function maybeStartAnomaly(
    ...
    const r = Math.random();
    if (r > 0.025) return null;  // 97.5 % — nothing happens
...
function advanceChannel(
    ...
    setEvent: (e: AnomalyEvent | null) => void,
): number {
    if (event === null) {
        // Normal walk
    ...
    const next      = Math.max(hardMin, Math.min(hardMax,
        parseFloat((prev + noise + reversion).toFixed(2))
    ));
...
function computeNextValues(s: RobotState): {
...
async function telemetryTick(): Promise<void> {
    ...
    for (const [, s] of states) {
        ...
        try {
            const res = await pool.query<{ recorded_at: Date }>(
                `INSERT INTO sensor_readings
                    (robot_id, temperature_c, vibration_mm_s, motor_current_a, pressure_bar, recorded_at)
                 VALUES ($1, $2, $3, $4, $5, NOW())
                 RETURNING recorded_at`,
                [s.robotId, vals.temperature_c, vals.vibration_mm_s, vals.motor_current_a, vals.pressure_bar]
            );
            const ts = res.rows[0]?.recorded_at;
            console.log(
                `[Telemetry] robot=${s.robotId} name=${s.robotName} ` +
